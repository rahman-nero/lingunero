...
```shell
make drop-database
docker compose exec -T mysql mysql -uroot -proot app < ./backend/backups/<<тут пишем имя backup-файла>>
```
...
- [x] Подправить мобильный дизайн. Меню подправить и сам чат.
- [ ] Скрол вниз при открытии чата
- [x] Удаление чата
- [ ] Кэширование и подтягивания кэша с базы при ответе LLM. Свой кэш для каждого чата
- [x] Доработать создание чата. Добавить иконку создания чата
